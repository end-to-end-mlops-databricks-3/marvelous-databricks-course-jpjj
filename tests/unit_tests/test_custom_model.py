...
def test_prepare_features(mock_custom_model: CustomModel) -> None:
    ...
    assert isinstance(mock_custom_model.pipeline.steps[1][1], CatBoostRegressor)
...
def test_register_model(mock_custom_model: CustomModel) -> None:
    ...
    client = MlflowClient()
    model_name = f"{mock_custom_model.catalog_name}.{mock_custom_model.schema_name}.doordash_eta_model_custom"
